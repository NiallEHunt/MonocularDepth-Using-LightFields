...
class AENet(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, num_filter, n_blocks=3, flag_step2=False):
        ...
        for i in range(self.n_blocks):
            self.add_module('conv_down_' + str(i + 1),
                            self.convsblocks(self.num_filter * (2 ** i) * 2, self.num_filter * (2 ** i) * 2, act_fnc))
        ...
        for i in range(self.n_blocks + 1):
            self.add_module('conv_up_' + str(i + 1),
                            self.upconvblock(self.num_filter * (2 ** (3 - i)) * 2, self.num_filter * (2 ** (3 - i)),
                                             act_fnc))
            self.add_module('conv_joint_' + str(i + 1),
                            self.convblock(self.num_filter * (2 ** (3 - i)) * 2, self.num_filter * (2 ** (3 - i)),
                                           act_fnc))
        ...
        if flag_step2:
            ...
            for i in range(self.n_blocks):
                self.add_module('conv_down2_' + str(i + 1),
                                self.convsblocks(self.num_filter * (2 ** i) * 2, self.num_filter * (2 ** i) * 2,
                                                 act_fnc))
            ...
            for i in range(self.n_blocks + 1):
                ...
                self.add_module('conv_joint2_' + str(i + 1),
                                self.convblock(self.num_filter * (2 ** (3 - i)) * 3, self.num_filter * (2 ** (3 - i)),
                                               act_fnc))
